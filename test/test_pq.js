  ...
  describe('CompleteHeap', () => {
    ...
    describe('inHeap', () => {
      it('should return false when i <= -1 or i >= n', () => {
        const heap = new CompleteHeap([1, 2, 3]);
        expect(CompleteHeap.inHeap(heap.size(), -1)).toBe(false);
        expect(CompleteHeap.inHeap(heap.size(), heap.size())).toBe(false);
      });

      it('should return true when -1 < i < n', () => {
        const heap = new CompleteHeap([1, 2, 3]);
        expect(CompleteHeap.inHeap(heap.size(), 0)).toBe(true);
      });
    });
    ...
    describe('parent', () => {
      it('should return parent rank', () => {
        const heap = new CompleteHeap([1, 2, 3]);
        expect(CompleteHeap.parent(1)).toBe(0);
        expect(CompleteHeap.parent(2)).toBe(0);
      });
    });
    ...
    describe('lastInternal', () => {
      it('should return last internal node', () => {
        ...
        expect(CompleteHeap.lastInernal(heap.size())).toBe(2);
      });
    });
    ...
    describe('leftChild', () => {
      it('should return rank of left child', () => {
        for (let i = 0; i < 3; i++) {
          expect(CompleteHeap.leftChild(i)).toBe((i << 1) + 1);
        }
      });
    });
    ...
    describe('rightChild', () => {
      it('should return rank of right child', () => {
        for (let i = 0; i < 3; i++) {
          expect(CompleteHeap.rightChild(i)).toBe((i + 1) << 1);
        }
      });
    });
    ...
    describe('leftChildValid', () => {
      it('should return true when left child is in heap', () => {
        for (let i = 0; i < 3; i++) {
          expect(CompleteHeap.leftChildValid(7, i)).toBe(true);
        }
      });

      it('should return false when left child is not in heap', () => {
        for (let i = 4; i < 7; i++) {
          expect(CompleteHeap.leftChildValid(7, i)).toBe(false);
        }
      });
    });
    ...
    describe('bigger', () => {
      it('should return rand of bigger one in array', () => {
        const array = [1, 2];
        expect(CompleteHeap.bigger(array, 0, 1)).toBe(1);
      });
    });
    ...
    describe('properParent', () => {
      it('should return rand of biggest one between parent, leftChild and rightChild', () => {
        const testProperParent = (pq, biggest) => {
          expect(CompleteHeap.properParent(pq, pq.length, 0)).toBe(biggest);
        };
